...
# Multi-GPU launcher for Qwen/Qwen2.5-7B-Instruct on ARC (classification) with tuned v3 hyperparameters.
#
# Changes vs previous:
# - spectral: gamma=0.1, tau=0.2, alpha=0.05, warmup_ratio=0.2, ramp_ratio=0.2 (warmup implemented in trainer)
# - r3f: lambda=0.05, noise_std=0.005
# - lora.r: 16
#
# Usage:
#   bash experiments/run_qwen7b_tuned_v3_multigpu.sh
#
...
WANDB_GROUP="${WANDB_GROUP:-qwen7b_tuned_v3}"
...
TASK_CONFIG="${TASK_CONFIG:-configs/classification/arc/qwen7b_tuned_v3.yaml}"
...
RESULTS_ROOT="${RESULTS_ROOT:-results/${DATASET_NAME}/qwen7b_tuned_v3}"
...
run_one() {
  ...
  python -m experiments.train_classification \
    ...
    --override "r3f.lambda=0.05" \
    --override "r3f.noise_std=0.005" \
    --override "smart.enabled=${smart}" \
    --override "spectral.enabled=${spectral}" \
    --override "spectral.alpha=0.05" \
    --override "spectral.gamma=0.1" \
    --override "spectral.tau=0.2" \
    --override "spectral.warmup_ratio=0.2" \
    --override "spectral.ramp_ratio=0.2" \
